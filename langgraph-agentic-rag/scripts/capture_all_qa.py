#!/usr/bin/env python3
"""Capture all questions and answers in a clean format for review"""
...
async def capture_all_qa():
    """Capture all questions and answers"""
    
    print("=== CAPTURING ALL QUESTIONS AND ANSWERS ===\n")
    ...
    print(f"Output will be saved to: all_questions_and_answers.txt\n")
    ...
    with open('all_questions_and_answers.txt', 'w') as output_file:
        # Write header
        output_file.write("LANGGRAPH AGENTIC RAG - ALL BUSINESS QUESTIONS AND ANSWERS\n")
    ...
    print("COMPLETE!")
    print(f"{'='*60}")
    print(f"All questions and answers saved to: all_questions_and_answers.txt")
    ...
    with open('all_qa_results.json', 'w') as f:
        json.dump(json_data, f, indent=2)
    
    print(f"JSON results saved to: all_qa_results.json")
